...
- Prefer SwiftUI. AppKit, CoreGraphics, and IOKit are used deliberately where SwiftUI has no equivalent — the `CGEventTap` + `IOHIDManager` keyboard hook in `KeyCapture.swift` is the prime example. Reach for them only when there is genuinely no SwiftUI/Swift-native path.
